    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                # Write results
                point_origin = (127.112, 37.1214124)
                x_old = point_origin[0]
                y_old = point_origin[1]
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        plot_one_box2(xyxy, im0, label=label, color=colors(c, True), line_thickness=line_thickness)
                        if save_crop:
                            save_one_box(xyxy, imc, file=save_dir / 'crops' / names[c] / f'{p.stem}.jpg', BGR=True)

                # print(int(xyxy[0]), int(xyxy[1]), int(xyxy[2]), int(xyxy[3])) #왼위:(0,0) -> 왼위가로, 왼위세로, 오른아래가로, 오른아래세로
                # print((int(xyxy[2])+int(xyxy[0]))/2, (int(xyxy[3])-int(xyxy[1]))/2) #(가로좌표, 세로좌표)
                ...
                real_scale = 30 * (math.tan(xdFoV / 2 - math.pi / 2 + cam_angle + flying_angle) + math.tan(
                    math.pi / 2 - (cam_angle + flying_angle) + xdFoV / 2))
                ...
                error_real_distance_coord = [error[0] * scaler[0], error[1] * scaler[1]]
